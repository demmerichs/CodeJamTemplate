...
def JudgePrint(s):
    ...
    if PRINT_INTERACTION_HISTORY:
        print("Judge prints:", s, file=sys.stderr)
...
def WrongAnswerExit(case_id, error_msg):
    print("Case #{} failed: {}".format(case_id, error_msg), file=sys.stderr)
    try:
        JudgePrint("-1")
    except IOError:
        print("Failed to print -1.", file=sys.stderr)
    # WaitForSubprocess(p)
    sys.exit(-1)
...
test_nbr = sys.argv[1:]
assert len(test_nbr) == 1
test_nbr = int(test_nbr[0])
...
JudgePrint(1)
if test_nbr not in list(range(len(CASES))):
    quit()

for i in [test_nbr]:
    L, U = CASES[i]
    if PRINT_INTERACTION_HISTORY:
        print("Test Case #{}:".format(i + 1), file=sys.stderr)
    JudgePrint("{} {}".format(L, U))
    try:
        # CheckSubprocessExit(p, i + 1)
        n = ReadN(L, U, sys.stdin.readline())
        if PRINT_INTERACTION_HISTORY:
            print("Judge reads:", n, file=sys.stderr)
        if isinstance(n, str):
        ...
        for _ in range(2 * n):
            ...
            if PRINT_INTERACTION_HISTORY:
                print("Judge reads:", link.strip(), file=sys.stderr)
            links.append(link)
        g = ReadGraph(links)
        if isinstance(g, str):
        ...
        for y in range(n):
            for z in sorted(list(h[y])):
                if y < z:
                    JudgePrint("{} {}".format(y + 1, z + 1))
        # CheckSubprocessExit(p, i + 1)
        resp = sys.stdin.readline()
        if PRINT_INTERACTION_HISTORY:
            print("Judge reads:", resp.strip(), file=sys.stderr)
        ...
        # CheckSubprocessExit(p, i + 1)
        WrongAnswerExit(i + 1, e)

extra_output = sys.stdin.readline()
# WaitForSubprocess(p)
if extra_output != "":
    print("Wrong Answer because of extra output:", file=sys.stderr)
    sys.stderr.write(extra_output)
    sys.exit(-1)
